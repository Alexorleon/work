...
SQL;
				...
				$sql = <<<SQL
				SELECT ALLTRAINING.ID, ALLTRAINING.TITLE, ALLTRAINING_B.STATUS FROM stat.ALLTRAINING, stat.ALLTRAINING_B WHERE ALLTRAINING.ID IN
				(SELECT ALLTRAININGID FROM stat.ALLTRAINING_B_TN WHERE ALLTRAINING_B_TN.DOLJNOSTID='$sotrud_dolj' AND ALLTRAINING_B.SOTRUDID='$temp_sotrud_id' AND ALLTRAINING_B.ALLTRAININGID=ALLTRAINING_B_TN.ALLTRAININGID) 
				AND ALLTRAINING.ALLTRAININGTYPEID=1 ORDER BY STATUS DESC
SQL;
				$array_instr = $db->go_result($sql);
