...
SQL;
			...
			die('<script>document.location.href= "'.lhost.'/commentAnswer.php?type_exam=1&q='.$idq.'"</script>'); // type_exam=1 означает предсменный экзаменатор pre_shift_examiner
		}else{
			//неправильно
...
SQL;
		...
		(SELECT ID, TEXT, TYPEQUESTIONSID, SIMPLEPHOTO FROM stat.ALLQUESTIONS WHERE ALLQUESTIONS.MODULEID='5'$certainID) WHERE rownum=1
SQL;
		$s_res = $db->go_result_once($sql);
